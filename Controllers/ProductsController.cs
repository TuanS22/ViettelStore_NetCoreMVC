...
namespace viettel_store.Controllers
{
    ...
    public class ProductsController : Controller
    {
        ...
        public IActionResult Categories(int? id, int? page)
        {
            ...
            if (CategoryId > 0)
                ...
                list_record = (from category in db.Categories
                               join category_product in db.CategoriesProducts on category.Id equals category_product.CategoryId
        ...
        public IActionResult AJaxSearch()
        {
            ...
            {
                str += "<li><a href='/Products/Detail/" + item.Id + "' style=\"text-decoration: none; color: #2382F1; font-size: 16px;\"><img src='/Upload/Products/" + item.Photo + "'>" + item.Name + "</a></li>";
            }    
            return Content(str);
        }
        ...
        public IActionResult Tag(int? id, int? page)
        {
            ...
            List<ItemProduct> list_products = (from item_tag in db.Tags
                                               join item_tag_product in db.TagsProducts on item_tag.Id equals item_tag_product.TagId
                                               join item_product in db.Products on item_tag_product.ProductId equals item_product.Id where item_tag.Id == tag_id select item_product).ToList();
